...
export default function Login() {
  ...
  // Check if user just registered from Register page
  const justRegistered = location.state?.emailJustRegistered;
  const [showConfirmationMsg, setShowConfirmationMsg] = useState(!!justRegistered);
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        setError(err.message || 'Identifiants invalides. Veuillez reessayer.');
      } finally {
        setSubmitting(false);
      }
    },
  ...
  // Don't render form if already logged in (redirect happens via useEffect)
  if (user) {
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <Loader2 className="w-8 h-8 text-amber-400 animate-spin" />
          ...
          <div className="relative z-10 p-6 md:p-8">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400 text-sm mt-1">
                Connectez-vous a votre compte BoutiKonect
              </p>
            </div>
            ...
            {showConfirmationMsg && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="p-4 mb-4 rounded-lg bg-amber-500/10 border border-amber-500/20"
              >
                <p className="text-amber-400 text-sm font-medium mb-1">
                  ✅ Compte cree avec succes !
                </p>
                <p className="text-amber-300/70 text-xs">
                  Un email de confirmation a ete envoye a <strong>{justRegistered}</strong>.
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-10 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all placeholder-gray-500"
                  />
                  <button
                    type="button"
            ...
            <div className="mt-6">
              ...
              <motion.button
                ...
                className="group relative w-full flex items-center justify-center gap-2.5 px-4 py-2.5 bg-white hover:bg-gray-100 text-gray-800 font-medium rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden"
